...
ping(Uri) ->
    ...
    channel_apply(Scheme, ChId,
        fun(Channel) ->
            {ok, Ref} = er_coap_channel:ping(Channel),
...
request_block(Channel, Method, ROpt, Block1, Content) ->
    {ok, Ref} = er_coap_channel:send(Channel,
        er_coap_message:set_content(Content, Block1,
            er_coap_message:request(con, Method, <<>>, ROpt))),
            ...
            case proplists:get_value(block2, Options) of
                {Num, true, Size} ->
                    ...
                    {ok, Ref2} = er_coap_channel:send(Channel,
                        er_coap_message:request(con, Method, <<>>, [{block2, {Num+1, false, Size}}|ROpt])),
...
channel_apply(coaps, {Host, Port}, Fun) ->
    {ok, Sock, Channel} = er_coap_dtls_socket:connect(Host, Port),
    % send and receive
    Res = apply(Fun, [Channel]),
    % terminate the processes
    er_coap_channel:close(Channel),
