...
-define(EXCHANGE_LIFETIME, 20000).
...
in_non({in, BinMessage}, State) ->
    case catch er_coap_message_parser:decode(BinMessage) of
...
out_non({out, Message}, State=#state{sock=Sock, cid=ChId}) ->
    ...
    BinMessage = er_coap_message_parser:encode(Message),
    ...
    case catch er_coap_message_parser:decode(BinMessage) of
        ...
        {error, Error} ->
            go_pack_sent(#coap_message{type=ack, method={error, bad_request},
                                       id=er_coap_message_parser:message_id(BinMessage),
                                       payload=list_to_binary(Error)}, State)
    end.

go_await_aack(Message, State) ->
    % we may need to ack the message
    BinAck = er_coap_message_parser:encode(er_coap_message:response(Message)),
...
go_pack_sent(Ack, State=#state{sock=Sock, cid=ChId}) ->
    ...
    BinAck = er_coap_message_parser:encode(Ack),
    Sock ! {datagram, ChId, BinAck},
...
await_pack({in, BinAck}, State) ->
    case catch er_coap_message_parser:decode(BinAck) of
