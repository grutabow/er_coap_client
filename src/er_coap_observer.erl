...
init([Client, Uri, Options]) ->
    ...
    {ok, Ref} = er_coap_channel:send(Channel,
        er_coap_message:request(con, get, <<>>, [{observe, 0}|ROpt])),
...
handle_call(shutdown, _From, State=#state{channel=Channel, ropt=ROpt}) ->
    {ok, Ref} = er_coap_channel:send(Channel,
        er_coap_message:request(con, get, <<>>, [{observe, 1}|ROpt])),
    Reply = er_coap_client:await_response(Channel, get, ROpt, Ref, #coap_content{}),
...
handle_info({coap_response, _ChId, Channel, Ref, Message=#coap_message{type=con}},
    ...
    {ok, _} = er_coap_client:ack(Channel, Message),
...
handle_response(Message=#coap_message{method={error, Code}}, State=#state{client=Client}) ->
    Client ! {coap_notify, self(), undefined, {error, Code}, er_coap_message:get_content(Message)},
    {stop, normal, State}.

request_more_blocks(Notify, {Num, true, Size}, Fragment,
        State=#state{client=Client, channel=Channel, ropt=ROpt}) ->
    {ok, Ref2} = er_coap_channel:send(Channel,
        er_coap_message:request(con, get, <<>>, [{block2, {Num+1, false, Size}}|ROpt])),
    receive
...
send_notify(Message=#coap_message{method={ok, Code}, options=Options},
    ...
    case proplists:get_value(observe, Options) of
        undefined ->
            ...
            Client ! {coap_notify, self(), undefined, {ok, Code}, er_coap_message:get_content(Message)},
            {stop, normal, State};
        N when LastSeq == undefined; N > LastSeq ->
            % report and stay subscribed
            Client ! {coap_notify, self(), N, {ok, Code}, er_coap_message:get_content(Message)},
